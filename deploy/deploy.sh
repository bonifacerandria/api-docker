...
if [ "$TAG" = "$PREVIOUS_TAG" ]; then
  echo " Le tag précédent est identique au tag déployé - pas de rollback possible automatiquement."
  exit 1
fi
...
if wait_for_health; then
  echo " Rollback réussi : $PREVIOUS_TAG est de nouveau en production. Le déploiement de $TAG a ÉCHOUÉ."
else
